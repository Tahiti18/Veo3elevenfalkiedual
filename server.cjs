...
const KIE_QUALITY_PATH = process.env.KIE_QUALITY_PATH || "/generate"; // many KIE installs use the same endpoint, model flag decides
const KIE_RESULT_PATHS = (process.env.KIE_RESULT_PATHS || "/result/:id,/status/:id").split(","); // we’ll try both

// FAL (Basic auth id/secret preferred)
...
async function submitAndMaybeWaitFAL(body, modelName) {
  ...
  const jid =
    ...
    j.id ||
    j.job_id ||
    (j.data && (j.data.request_id || j.data.id)) ||
    null;
...
async function submitAndMaybeWaitKIE(body, modelName) {
  ...
  // Short poll – try multiple result paths
  for (let i = 0; i < 5; i++) {
    await sleep(i === 0 ? 3000 : 5000);
    for (const pat of KIE_RESULT_PATHS) {
      const pathTmpl = pat.trim();
      if (!pathTmpl) continue;
      const p = pathTmpl.replace(":id", encodeURIComponent(jid));
...
// Raw stream (audio/mpeg) for immediate playback
